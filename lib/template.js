...
(function (Hogan) {
  ...
  Hogan.Template.prototype = {
    ...
    d: function(key, ctx, partials, returnFound) {
      ...
      if (key === '.' && isArray(ctx[ctx.length - 2])) {
        ...
        for (var i = 1; i < names.length; i++) {
          ...
          if ((this.options.allowEmpties && !!found) ||
           (!this.options.allowEmpties && found !== undefined)) {
            cx = val;
            val = found;
          } else {
            val = this.options.defaultEmptyValue || '';
          }
        }
      }
    ...
    f: function(key, ctx, partials, returnFound) {
      ...
      if (!found) {
        return (returnFound) ? false : (this.options.defaultEmptyValue || '');
      }
